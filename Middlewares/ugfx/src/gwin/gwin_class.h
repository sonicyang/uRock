...
#define GWIN_FLG_NEEDREDRAW				0x00004000			// @< uRedraw is needed but has been delayed
...
typedef struct gwinVMT {
	...
	void (*uRedraw)		(GWindowObject *gh);			// @< The GWIN redraw routine (optional)
...
/** @} */
	 ...
	 * @note	A widget must have a redraw function. Use @p _gwidgetuRedraw().
	 ...
	 * @note	A container must have a gwin redraw function. Use @p _containeruRedraw().
	 * @note	If toggleroles != 0, ToggleAssign(), ToggleGet() and one or both of ToggleOff() and ToggleOn() must be specified.
	 * @note	If dialroles != 0, DialAssign(), DialGet() and DialMove() must be specified.
	 * @{
	 */
	...
	typedef struct gwmVMT {
		...
		void (*uRedraw)		(GHandle gh);							// @< A window needs to be redraw (or undrawn)
...
/**
 * @brief	uRedraw the window after a status change.
 *
 ...
 */
typedef enum GuRedrawMethod { REDRAW_WAIT, REDRAW_NOWAIT, REDRAW_INSESSION }	GuRedrawMethod;
...
bool_t _gwinWMAdd(GHandle gh, const GWindowInit *pInit);
	...
	/**
	 * @brief	uRedraw the Widget object (VMT method only)
	 *
	 * @param[in]	gh		The widget to redraw
	 *
	...
	/**
	 * @brief	uRedraw the Container object (VMT method only)
	 *
	 ...
	 * @notapi
	 */
	#define _gcontaineruRedraw		_gwidgetuRedraw
